...
class ChatConnector(input: InputStream, output: OutputStream, var userName: String = "") : Runnable, MessageObserver {
    ...
    private fun addMessageToChatHistory(userInput: String) {
        val chatMessage = ChatConsole(userInput, userName, LocalDateTime.now())
    ...
    private fun addMessageToPrivateHistory(text: String, fromUser: String, toUser: String) {
        val chatMessage = ChatConsole(text, userName, LocalDateTime.now())
        ...
        when {
            ...
            userInput.startsWith(":addmember ") -> addMember(userInput)
                ...
                ":messages" -> out.println(
                    ChatHistory.getMessages().joinToString(
                        prefix = "",
                        postfix = "",
                        separator = "\r\n"
                    )
    ...
    private fun isMessageForGroupOrUser(userInput: String) =
        userInput[0] == '@' //check whether input is a group message
...
//        }
    ...
    override fun incomingMessage(message: ChatConsole) {
        out.println("${message.message} from ${message.userName}")
    }

    override fun getUsername() = userName

    override fun incomingGroupMessage(group: String, message: ChatConsole) {
        out.println("from @$group: " + "${message.message} from ${message.userName}")
    }

    override fun incomingPrivateMessage(userName: String, message: ChatConsole) {
