...
class ChatServer {
    ...
    fun createLocalPortAndEstablishCommunication() {
        val serverSocket = ServerSocket(23) //choose a port number(), or 0, then get any assigned port number
    ...
    private fun startInterpreter(socket: Socket) {
        ...
        val interpreter = ChatConnector(socket.getInputStream(), socket.getOutputStream())
